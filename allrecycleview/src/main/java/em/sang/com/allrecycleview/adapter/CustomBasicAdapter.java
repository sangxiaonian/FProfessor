...
public class CustomBasicAdapter<T> extends BasicAdapter {
    public CustomBasicAdapter(Context context, List<T> lists, int itemID, DefaultAdapterViewLisenter<T> lisenter) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (position < heards.size()) {
            ...
            ((CustomHolder) holder).initView(position - heards.size(),context);

        } else {
            ((CustomHolder) holder).initView(position - heards.size() - lists.size(),context);
        }
